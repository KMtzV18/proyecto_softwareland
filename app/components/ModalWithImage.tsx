...
export default function ModalWithImage({src, alt}: ModalProps) {
    ...
    return (
        <div className="flex justify-center p-4">
            <button onClick={mostrarModal} className="m-2 bg-gray-600 hover:bg-gray-800 p-2 text-white font-bold rounded transition-colors cursor-pointer">Mostrar modal</button>
            <Modal show={showModal} onClose={ocultarModal}>
                ...
                <ModalFooter className="flex justify-end gap-3 border-t border-gray-100 dark:border-gray-800 p-4 bg-gray-50 dark:bg-gray-900 rounded-b-lg">
                    <button onClick={ocultarModal} className="px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded transition-colors cursor-pointer">Cerrar</button>
                </ModalFooter>
            </Modal>
        </div>
    );
